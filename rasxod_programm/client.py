...
class ModbusClient:
    ...
    def __init__(self, host=None, port=None, unit_id=None, timeout=None,
        ...
        self.__unit_id = 0
        self.__timeout = 5.0                # socket timeout
    ...
    def read_holding_registers(self, reg_addr, reg_nb=1):
        ...
        data_l = len(data)
        try:
            send_l = self.__sock.sendall(data)
            response = self.__sock.recv(data_l+1)
            return struct.unpack('>L',response[3:-2])
        except socket.error:
            print("ERROR")
        # handle send error
    ...
    def _mbus_frame(self, fc, body):
        ...
        if self.__mode == const.MODBUS_TCP:
            ...
            # print((f_mbap[-1:] + f_body)+struct.pack('<H',libscrc.modbus((f_mbap[-1:] + f_body))))
            return ((f_mbap[-1:] + f_body)+self._add_crc((f_mbap[-1:] + f_body)))
